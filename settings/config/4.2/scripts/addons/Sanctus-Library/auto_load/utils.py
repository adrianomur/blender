...
def resolve_unique_naming(duplicate: str, pool: list[str], count_up: bool = False):
    if duplicate not in pool:
        return duplicate
    has_numbering, index = name_numbering(duplicate)
    stem = duplicate[:-4] if has_numbering else duplicate
