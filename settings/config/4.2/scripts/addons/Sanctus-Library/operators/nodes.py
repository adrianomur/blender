...
class ConnectSmartModules(base_ops.SanctusOperator):
    ...
    def get_asserts(cls, context: bt.Context):
        ...
        yield al.OperatorAssert(lambda: len(selected_nodes) >= 2, 'At leat 2 nodes selected')
        yield al.OperatorAssert(lambda: all(isinstance(x, bt.ShaderNodeGroup) for x in selected_nodes), 'Selected nodes are group nodes')
