...
class LibraryManager:
    ...
    dynamic_menus: dict[str, al.MenuBuilder]
    loaded: bool
...
def _filter_glob(search_results: typing.Generator[Path, None, None]):
    return [x for x in search_results if x.is_dir() or x.suffix in VALID_FILE_TYPES]
...
class RawFileStructure(dict[str, Union["RawFileStructure", Path]]):
    ...
    def get_category_paths(self, parent_path: Path = Path()):
        yield parent_path
