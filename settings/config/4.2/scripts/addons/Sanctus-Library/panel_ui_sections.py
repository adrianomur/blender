...
def draw_developer_tool(asset: lm.lib.Asset, layout: bt.UILayout):
    from . import meta_data
    meta_data.SetMetaData(
        asset_path=str(asset.asset_path)
    ).draw_ui(layout)
    developer_tool.draw_meta_data_preview(layout, asset.asset_name, asset.meta)
    developer_tool.CopyItemTemplateDocumentation(asset_path=str(asset.asset_path)).draw_ui(layout)
...
def draw_item_popup(layout: bt.UILayout, path: Path):
    ...
    layout.template_icon_view(attrs, str(path), show_labels=True, scale=1.0, scale_popup=pref.get().interface().asset_thumbnail_scale())
    col = al.UI.column(layout, align=True)
...
class SanctusPanelSection:
    ...
    def draw_ui(cls, layout: bt.UILayout, context: bt.Context) -> None:
        ...
        manager = lm.MANAGER
        sanctus_filters = filters.SanctusLibraryFilters(al.get_wm())
        ...
        prefs = pref.get()
        if prefs.get_developer_mode():
            draw_developer_tool(asset_instance.asset, layout)
    ...
    def get_light_version_asset_count(cls):
        return sum((asset.preset_count if asset.has_presets else 1) for path, asset in lm.MANAGER.all_assets.items() if cls.asset_class in path.parts)
...
class SanctusMaterialSection(SanctusPanelSection):
    ...
    def draw_details(cls, layout: bt.UILayout, context: bt.Context):
        if not context.engine in ['BLENDER_EEVEE', 'CYCLES']:
            return
        ...
        global_mat_props = GlobalProperties(al.get_wm())
        prefs = pref.get()

        if prefs.interface().display_material_slots():
            if global_mat_props.material_slots_expandded.draw_as_dropdown(layout, 'Material Slots:', 'Material Slots...'):
        ...
        if global_mat_props.properties_expanded.draw_as_dropdown(layout, 'Active Material Properties:', 'Active Material Properties...'):
            cls.draw_material_properties(layout, obj, engine)


class SanctusGNAssetsSection(SanctusPanelSection):
    asset_class: str = 'gntools'
    name: str = 'GN Assets'
    description: str = ''
    ...
    def draw_gn_inputs_blender4(cls, layout: bt.UILayout, mod: bt.NodesModifier):
        ...
        for tree_item in mod.node_group.interface.items_tree:
            ...
            if isinstance(tree_item, bt.NodeTreeInterfaceSocketString) and node_utils.input_name_is_category(tree_item.name):
                ...
                continue
            if show:
                al.UI.prop_custom(al.UI.row(layout), mod, tree_item.identifier, al.UIOptionsProp(text=tree_item.name))


class SanctusDecalsSection(SanctusPanelSection):
    asset_class: str = 'stickers'
    name: str = 'Decals'
    description: str = ''
...
class GlobalProperties(al.PropertyGroup):
    ...
    material_slots_expandded = al.BoolProperty(name='Material Slots Expanded')
