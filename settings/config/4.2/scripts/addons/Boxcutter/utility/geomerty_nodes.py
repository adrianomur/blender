...
class NodeModifier():
    ...
    def builtin_geometry_nodes_path(cls) -> pathlib.Path:
        ...
        path = pathlib.Path(bpy.app.binary_path).parent # exec folder on mac and win


        if sys.platform == 'darwin':
            path = path.parent / 'Resources'

        elif sys.platform == 'linux':
            path = pathlib.Path(bpy.app.binary_path)

        path = path / ver / 'datafiles' / 'assets' / 'geometry_nodes'
