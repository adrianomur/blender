...
@al.depends_on(SL_PT_togglable_ui)
@al.register
...
class SL_PT_baking(bt.Panel):
    ...
    def draw(self, context: bt.Context):
        ...
        baking_queue = baking.queue.BakingQueue(context.object)
        baking.ui.BakingQueueDrawer(baking_queue, l.box(), context).draw()
        manager = baking.texture_sets.TextureSetManager(context.scene)
        baking.ui.TextureSetManagerDrawer(manager, l.box(), context).draw()
        DocButtons(l).baking()


@al.register
class SL_PT_ShaderEditor(bt.Panel):
...
@al.register
class SL_PT_MaterialEditor(bt.Panel):
    ...
    def draw(self, context: bt.Context):
        ...
        if pref.get().get_developer_mode():
            from . import developer_tool
            developer_tool.CopyGroupNodeValuesToNodeGroup().draw_ui(l, al.UILabel("Copy Node Values To Group"))
...
@al.register
class SL_PT_CompositorEditor(bt.Panel):
