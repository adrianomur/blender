...
class Operator(props.AnnotatedObject, bt.Operator):
    ...
    def poll(cls, context: bt.Context) -> bool:
        return len(list(cls.check_asserts(context))) == 0
    ...
    def delegate_description(cls, context: bt.Context, properties: bt.OperatorProperties) -> str:
        ...
        if cls.al_description_failed != '':
            return cls.al_description_failed
        text = cls.al_asserts_header
        if not text == '':
            text += '\n'
        return text + '\n'.join(list(f'- {l}' for l in cls.check_asserts(context)))
