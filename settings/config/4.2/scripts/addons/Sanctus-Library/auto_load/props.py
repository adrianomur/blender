...
class BoolProperty(StandardProperty[bool]):
    ...
    def draw_as_icon_switch(self, layout: bt.UILayout, icon_true: ui.IconParam, icon_false: ui.IconParam, hide_highlighting: bool=True):
        return ui.UI.prop_bool_icon_switch(layout, self.obj, self.data_attr, icon_true, icon_false, hide_highlighting)
...
class PathProperty(StandardProperty[str]):
    ...
    def absolute(self):
        ...
        if self.fallback == enums.BPathFallbackType.NONE and (not bpy.data.is_saved):
        ...
        if self.fallback == enums.BPathFallbackType.NONE:
            ...
            print(f'Blender Absolute Path: {str(p)}')
            return p
...
class ContextProperty(Property[PropType]):
    ...
    def get_from_context(self, context: bt.Context, uid: int, error: bool = True) -> PropType:
        ...
        if not hasattr(context, key) and error:
            raise AttributeError(f'Context Property "{self.attr_name}" got invalid value. Has to be bpy.types.AnyType')
...
class PropertyGroup(typing.Generic[ParentType], AnnotatedObject, BClassMappedObject, bt.PropertyGroup):
    ...
    def __new__(cls, obj: ParentType):
        prop = cls.get_meta_prop(obj)
        return getattr(obj, prop.attr_name)

    def __init__(self, obj: ParentType):
        pass  # Skipped because __new__ does not return an Instance of this property group
