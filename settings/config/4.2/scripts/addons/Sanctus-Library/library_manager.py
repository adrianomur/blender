...
def load_raw_structure():
    ...
    #add custom decal categories
    custom_decal_dir_prop = pref.get().decals().custom_decal_folders
    for custom_dir in custom_decal_dir_prop():
        try:
            if not custom_dir.directory.absolute.exists():
                continue
        except OSError:
            continue
        ...
        MANAGER.raw_files.add_directory(
            custom_dir.directory.absolute, 
            Path(f'stickers'), 
            Path(custom_dir.category_name().lower()), 
            recursive=False, 
            replace=False,
...
def generate_assets():
    ...
    for category_path in MANAGER.raw_files.get_category_paths():
        ...
        for asset in lib.get_assets_from_paths(list(cat.get_files()), category_path).values():
...
def register_library_attributes(register: bool):
    if register:
        favorites = pref.get().favorites().keys()
        ...
        get_library_attributes().resolve_dirty_selections(debug=True)
...
def read_loaded_images():
    ...
    favorite_instance_paths: typing.Iterable[str] = preferences.get().favorites().keys()
    previews = {x: MANAGER.icon_collection[x] for x in icon_ids}
    for i in (v for k, v in previews.items() if k in favorite_instance_paths):
        img_tools.make_icon_favorite(i, MANAGER.icon(Path('icons/star')))
    return 0.01
